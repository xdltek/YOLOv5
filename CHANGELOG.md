...
### Added
- Added `yolov5_rgb_image_sequence_demo` for single-process sequential RGB/BGR image inference while reusing one YOLOv5 pipeline instance.
...
- Restored preprocessing workspace to regular `rtMallocSram`/`rtFreeSram` because virtual SRAM is limited to graph-related runtime APIs.
